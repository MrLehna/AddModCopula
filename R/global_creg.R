...
#'  # Weibull
#'  p1=list(
#'    scale = cbind(ones, abs(helgoland_wind$ws[9:(obs-1)])),
#'    shape_a = cbind(ones, abs(helgoland_wind$ws[9:(obs-1)]))),
#'  # von Mises dist
#'  p2=list(
#'    mu = cbind(ones, helgoland_wind$dwd[9:(obs-1)]),
#'    kappa = cbind(ones, helgoland_wind$dwd[9:(obs-1)])),
#'  # Copula
#'  theta = cbind(ones)
#')
#'

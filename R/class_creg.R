...
#' @import R.methodsS3
R.methodsS3::setMethodS3("summary","creg", function(x,param=FALSE, ...){
...
#' #'  copula regression
#' #' @import R.methodsS3
#' R.methodsS3::setMethodS3(name="print",class="creg", function(x,...){
#'   if (class(x)!="creg"){stop("The argument has to be from the class creg")}else{
...
R.methodsS3::setMethodS3("AIC","creg",function(x, ...,k=2){
    ...
    r <- 2 * ncol(x$Transformed)- k *x$result$Likelihood;
    return(r);
  },createGeneric=TRUE,exportGeneric=TRUE,overwrite=TRUE,export=TRUE)
